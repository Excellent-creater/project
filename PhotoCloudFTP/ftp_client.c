...
void get_FTPClient(FTPClient* ftp,const char* filename)
{
   ...
   sprintf(ftp->buf, "SIZE %s\n", filename);
    ...
    // 获取本地文件的大小
    size_t cfile_size = lseek(file_fd, 0, SEEK_END);

    // 如果本地文件存在且大小与服务器文件不同，询问是否覆盖
    if (cfile_size > 0 && cfile_size != sfile_size)
    {
        printf("已有同名文件存在，是否覆盖?（y/n): \n");
        if ('y' != getch())
        {
